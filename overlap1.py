...
from normalization import norm_lines, norm_spaces


def go(output_file, input_files, num_overlaps):
    output = set()
    for fin in input_files:
        norm_lines(fin[:-4] + "_lines.txt", fin)
        norm_spaces(fin[:-4] + "_prep.txt", fin[:-4] + "_lines.txt")
        lines = open(fin[:-4] + "_prep.txt", 'rt', encoding="utf-8").readlines()
